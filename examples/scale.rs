...
fn main() {
    ...
    let guards = timely_communication::initialize(config, |allocator| {

        println!("Worker {:?}/{:?} started", allocator.index(), allocator.peers());
        ...
        println!("Comms active: {:?}", state.comms.ether.is_some());
    ...
    pub fn load_rows(name: &str, filename: &str, state: &mut datatoad::types::State) {
        ...
        if let Ok(file) = std::fs::File::open(filename) {
            ...
            if let Some(record) = reader.read_byte_record().unwrap() {
                ...
                while let Some(record) = reader.read_byte_record().unwrap() {
                    if counter % state.comms.peers() == state.comms.index() {
                        for term in record.iter() {
                            let num = term.iter().fold(0u32, |n,b| n*10 + ((b-48) as u32));
                            bytes.extend(num.to_be_bytes());
                        }
                    ...
                    counter += 1;
                    if (counter * 4 * arity) >= 4_000_000_000 {
                        state.extend_facts(&atom, to_forest(&mut bytes, arity).into());
                        bytes.clear();
                        counter = 0;
                    }
                }
